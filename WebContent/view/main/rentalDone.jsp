...
<HTML>
	...
	<body>
	    ...
	    <main style="text-align:center; padding-top: 30px;" class="container">
	    	<c:if test="${param.OUTPUT == 'NONE' }">
	    		<h3>대여할 수 없습니다. </h3>
	    	</c:if>
	    	<c:if test="${param.OUTPUT == 'FAIL' }">
	    		<h3>존재하지 않는 ID입니다. </h3>
	    	</c:if>
	    	<c:if test="${param.OUTPUT == 'SUCCESS' }">
	    		<h3>대여되었습니다.</h3>
	    	</c:if>
	    	<button class="btn btn-secondary" style="width:20%" onClick="window.history.back();">돌아가기</button>
	    </main>
		<footer>
